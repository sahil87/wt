...
- **Gate C1**: shll fab change `ttoa` (`260911-ttoa-hexokit-banner-and-policy`), PR [shll#98](https://github.com/sahil87/shll/pull/98) — open draft, pipeline through review-pr done 2026-09-11. Its diff to `docs/site/standards/readme-extraction.md` is the source of the revised blockquote text (see What Changes).
- **Installed `shll`** (`shll standards readme-extraction`) still prints the *pre-C1* blockquote (`> Part of the [shll toolkit](https://shll.ai) — see all projects there.`) because shll#98 is unreleased. The user's gate wording ("PR up, review-pr done") makes the PR-branch text authoritative for this change — recorded as Assumption 2.
...
- **Files changed in this repo**: `README.md` (1 line). Hydrate: `docs/memory/wt-cli/toolkit-standards-conformance.md` (+ regenerated `docs/memory/wt-cli/index.md` / `log.md` via `fab docs-index`).
...
| 2 | Confident | The revised standard text is taken from the shll#98 PR branch, not from the installed `shll standards readme-extraction` (which still prints the pre-C1 line) | User's gate is "PR up, review-pr done", not "merged/released"; the conformance memory's "runtime enumeration is authoritative" rule assumes the release carries the standard, which is not yet true. Banner text is pipeline-free (D14), so ordering is safe. Receipt records the change/PR, not a shll version | S:80 R:90 A:70 D:70 |
