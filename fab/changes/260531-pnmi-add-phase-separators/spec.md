# Spec: Phase Separators in Worktree Creation Output

**Change**: 260531-pnmi-add-phase-separators
**Created**: 2026-05-31
**Affected memory**: `docs/memory/wt-cli/create-output-phases.md`
...
|---|-------|----------|-----------|--------|
| 1 | Certain | Scope = all three phases (Git, Init, Open) | Confirmed from intake #1 — user explicitly chose "All phases" | S:98 R:80 A:90 D:95 |
| 2 | Certain | Separators on stderr only for `wt create`; stdout final-path line byte-identical | Confirmed from intake #2; reinforced by `launcher-contract.md` §6 stability guarantee on stdout | S:98 R:70 A:95 D:95 |
| 3 | Certain | NO_COLOR form keeps a plain-ASCII labeled rule (`-- Git --`); colored form uses unicode `──` | Confirmed from intake #3; verified errors.go `init()` already blanks the color vars on NO_COLOR | S:97 R:85 A:90 D:95 |
| 4 | Certain | Init separator label = `Init (<resolved cmd>)`, derived from `InitScriptPath()`/resolved cmd | Confirmed from intake #4 | S:96 R:85 A:90 D:92 |
| 5 | Certain | `PhaseSeparator(label)` helper in errors.go, fixed 40-col, reuses NO_COLOR vars, no trailing newline | Confirmed from intake #5 (clarified). Verified against errors.go structure | S:95 R:80 A:90 D:80 |
| 6 | Certain | Init separator owned by the runner (crud.go + init.go), single emission point | Confirmed from intake #6 (clarified). Verified both call sites route through the runner / resolve the same value | S:95 R:75 A:90 D:85 |
| 7 | Certain | Existing substrings preserved (`Created worktree:`, `Running worktree init`, `Worktree init complete`, `Open in:`); new tests for separators | Confirmed from intake #7 (clarified). Grep verified assertions are substring/combined-stream based | S:95 R:60 A:90 D:80 |
| 8 | Certain | `wt init` realigned: banner + `cmd.Stdout`/`cmd.Stderr` move stdout → stderr; in scope | Clarified — user confirmed after stdout/stderr convention discussion; init_test asserts on combined streams so it stays green | S:95 R:65 A:88 D:90 |
| 9 | Certain | Rule glyph `──`/`--`, fixed ~40 cols, bold label | Clarified — user confirmed default proposal | S:95 R:90 A:75 D:90 |
| 10 | Certain | Open separator (and any phase separator) is emitted only when its phase produces output (e.g. skipped when `--worktree-open=skip`); Git always emits (summary always prints), Init emits only when a command runs | Clarified — generalized the per-phase emission rule against create.go control flow (deferred summary at L264 always prints; runner short-circuits to RenderWarning with no separator; open block gated on `worktreeOpen != "skip"`), resolving the apparent contradiction with the always-emitted Git separator | S:95 R:85 A:88 D:90 |
