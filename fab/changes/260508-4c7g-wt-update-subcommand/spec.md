...
2. **Brew-not-found returns `nil` from cobra wrapper**:
   - *Why*: hop suppresses double-printing via an `errSilent` sentinel; wt's `main.go` has no such sentinel. The cleanest port is for the `update.Run` function to write the user-visible hint to its `errOut` parameter, return a sentinel error (e.g., `update.ErrBrewNotFound`), and have `cmd/wt/update.go`'s `RunE` map that sentinel to `nil`. The user sees one hint line; the binary still exits 1 because we explicitly call `os.Exit(ExitGeneralError)` from the cobra wrapper after printing OR we accept exit 0 if treating the missing-brew case as a "manual update required" no-op.
   - *Decision detail*: To preserve the documented exit-code semantic ("brew failure → 1"), the cobra wrapper SHALL detect the sentinel and explicitly call `wt.ExitWithError(wt.ExitGeneralError, ...)` after the hint has been printed by `internal/update`. This bypasses cobra's automatic error printing while preserving the exit code. Implementation detail belongs in `cmd/wt/update.go`'s `RunE`.
...
| 5 | Certain | Brew-not-found returns nil from cobra wrapper after explicit `ExitWithError` | Confirmed from intake #5 (clarified); refined to: cobra wrapper calls `wt.ExitWithError(ExitGeneralError, ...)` directly to bypass cobra's error print, preserving exit-code semantics. Documented in Design Decision 2. | S:95 R:80 A:75 D:80 |
