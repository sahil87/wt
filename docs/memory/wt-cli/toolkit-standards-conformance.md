---
type: memory
description: "The sahil87-toolkit-standards conformance baseline for `wt` — which standards version was audited (shll v0.0.23), the per-standard verdicts, what was fixed vs deferred (principle №5's `--dry-run` gap since closed by 260717-p5m9), and the re-audit trigger (any CLI-surface / README / docs-site change must be re-checked against the standards per the constitution's Toolkit Standards article)."
---
...
- **`principles`** (foundation, №1–№10): PASS with two fixes and two deferrals.
  ...
  - №2 (stdout=data / stderr=diagnostics): PASS for the machine-contract commands (`create`, `go`, `init`, `list --json`). The one gap at audit time — `wt delete`'s non-warning human copy still printing to stdout — was **deferred to `[ohwb]` and is now closed** by `260717-ohwb-delete-copy-stderr-realign`: ALL of `delete.go`'s non-error human copy was realigned stdout → stderr, so `wt delete` now conforms to №2. Its stdout is empty on every live path; the sole stdout output is the `--dry-run` preview (`Dry run` header + `Would …` lines) added concurrently by `260717-p5m9` — the machine result the caller asked for (see [delete-dry-run-contract](/wt-cli/delete-dry-run-contract.md)). See [create-output-phases](/wt-cli/create-output-phases.md) § `wt delete`'s entire non-error human copy is on stderr. (The shared interactive menu rendering, still on stdout, was explicitly out of scope for that change — cross-command infrastructure with its own `menu-navigation-contract.md`.)
  ...
  - №5 (visible mutation boundaries): read/write is clear from names and `wt delete` requires consent. The deferred `--dry-run` gap is now **closed in `260717-p5m9`** — `wt delete` gained a `--dry-run` preview sharing the live code path (all decision logic runs live, only leaf mutations are gated), so principle №5 is now **conformant for `wt delete`** (see [delete-dry-run-contract](/wt-cli/delete-dry-run-contract.md)). The destructive-path audit `[p5m9]` also ran confirmed that `wt delete` is the **only** command with wt-owned destructive writes — `create` rollback is self-scoped, init/open are delegated/side-effecting-not-destructive, `update` is delegated to brew, and `go`/`list`/`shell-init` are read-only — so **no other command required `--dry-run`** (full dispositions in [delete-dry-run-contract](/wt-cli/delete-dry-run-contract.md) § destructive-path audit).
...
*Introduced by*: `260717-6end-toolkit-standards-conformance`
*Update*: the `--dry-run` deferral `[p5m9]` landed as its own restructuring-sized change (`260717-p5m9`) exactly as this boundary intended — see [delete-dry-run-contract](/wt-cli/delete-dry-run-contract.md).
...
- Deferrals: `fab/backlog.md` — all three audit deferrals are now closed: `[v7xy]` (`wt skill` adoption) **resolved in `260717-v7xy`**, `[p5m9]` (`wt delete --dry-run`) **closed by `260717-p5m9`** (see [delete-dry-run-contract](/wt-cli/delete-dry-run-contract.md)), and `[ohwb]` (`wt delete` stdout→stderr realignment) **closed by `260717-ohwb-delete-copy-stderr-realign`** (the №2 gap above).
