...
- The Git separator MUST stay inside the existing deferred-summary emission, **before** the init-phase `signal.Reset` (`create.go:282`). It MUST NOT introduce new I/O or a prompt inside the tight reinstall window between `git worktree add` returning and `signal.Reset` (see `init-failure-contract.md` "SIGINT during init").
