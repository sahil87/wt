...
> Post-implementation behavior capture for the `wt create` / `wt init` init-failure behavior.
> Source changes: `260516-g5e7`, `260602-z4p7`, `260626-n6ma`, `260705-irnt`
...
- The resolver leaves `Dir`, `Stdout`, `Stderr` unset on the returned `*exec.Cmd` — callers wire those themselves because `wt init` and `wt create` use different working directories; both stream init diagnostics uniformly to stderr (`cmd.Stdout = cmd.Stderr = os.Stderr`), so only the working directory differs between them (260531-pnmi; see `create-output-phases.md`).
...
- **`cmd/wt/create.go` exits 7 on ALL init-failure paths** — non-interactive, interactive open-anyway *Yes* (even when the open succeeds), and interactive *No*. Exit 7 is NOT issued inline at the banner: an `initFailed bool` flag is set in the init-failure branch (`create.go:376`), and a single end-of-function guard `if initFailed { os.Exit(wt.ExitInitFailed) }` (`create.go:476–478`) fires after the Open phase (260626-n6ma). That guard is placed **before** `fmt.Println(wtPath)` / `return nil`, so a *successful* open-anyway open can never downgrade the exit to 0 and erase the init-failure signal. The lone inline `os.Exit(wt.ExitInitFailed)` is the non-interactive branch (`create.go:400`), reached only when no prompt is offered.
- `cmd/wt/init.go` (the `wt init` subcommand) also exits via `os.Exit(wt.ExitInitFailed)` on init-script non-zero exit. Returning the error to Cobra would map to `ExitGeneralError = 1`; the explicit `os.Exit` ensures both command paths emit the typed code. (`wt init` has no open-anyway prompt — it does not call `PrintInitFailureBanner`.)
...
### Default-not-applicable run-time skip bypasses the failure contract

> The one carve-out from "`ExitInitFailed = 7` on EVERY path" above (260705-irnt). When the **built-in default** init script `fab sync` runs in a repo that is not fab-managed and exits fab-kit's `ExitNotManaged = 3`, the whole failure contract is bypassed: no `ExitInitFailed`, no kept-worktree banner, no open-anyway prompt, no `Worktree init complete.` line, exit 0, and `wt create`'s Open phase proceeds exactly as on init success.
...
- `wt create` AND `wt init` capture the controlling terminal's foreground process group (`tcgetpgrp` on `os.Stdin.Fd()`) **immediately before** running the init child, and reclaim it (`tcsetpgrp` back to that captured pgrp — `wt`'s own group, since `wt` is the foreground at capture time) **after** the child returns, on ALL exit paths:
  - **`wt create`** (`cmd/wt/create.go`):
    ...
    - **Unified post-init reclaim/teardown** (260626-n6ma): teardown + reclaim run **ONCE, unconditionally**, in a single post-init block immediately after `RunWorktreeSetupWithObserver` returns — `signal.Stop(initSigCh)` + `close(initSigCh)` (`create.go:355–356`) then `reclaimTerminalForeground(ttyFd, wtPgid)` (gated on `reclaimTTY`, `create.go:363–365`) — **before** the init-failure banner, the open-anyway prompt, AND the Open phase separator + menu render. This single reclaim is the load-bearing one for every downstream TTY write: it covers the success-path Open menu, the init-failure banner (itself a TTY write), the open-anyway prompt, and the Open menu reached via the open-anyway *Yes* fall-through.
...
- The runner contains NO confirmation prompt. Callers that want a prompt MUST call `ConfirmYesNo("Initialize worktree?")` themselves before invoking the runner — see § Design Decisions (The init confirm prompt lives in `cmd/`, never in the runner).
...
### The init confirm prompt lives in `cmd/`, never in the runner

**Decision**: the "Initialize worktree?" confirm is issued by `cmd/wt/create.go` BEFORE `RunWorktreeSetup*` is invoked; the runner itself never prompts (no `mode` parameter).
**Why**: the init-phase SIGINT handler must be installed AFTER the prompt completes — a prompt inside the runner would have Ctrl-C during the prompt consumed by the init handler with no init child to target (deadlock).
**Rejected**: a prompt inside the runner gated by a `mode` parameter (the deadlock above; flagged in the Copilot review of PR #7).
*Introduced by*: `260516-g5e7-wt-create-init-failure-dx`
...
### Open-anyway via flag + Open-phase fall-through, not a new open codepath
